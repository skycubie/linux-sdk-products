...
#Product Variants
CB_PRODUCT_ROOTFS_IMAGE=${CB_PACKAGES_DIR}/fedora-20-minimal.ext4
CB_PRODUCT_ONLY_KERNEL=0
U_BOOT_WITH_SPL=${CB_PACKAGES_DIR}/u-boot-cb2.bin
...
cb_build_release()
{
    ...
    if [ -f ${CB_PACKBUILD_DIR}/livesuit_cubieboard2.img ]; then
        echo "copy livesuit image"
	mv  ${CB_PACKBUILD_DIR}/livesuit_cubieboard2.img ${CB_RELEASE_DIR}/${CB_PRODUCT_NAME}-nand.img
        ...
        cp ${CB_KBUILD_DIR}/.config ${CB_RELEASE_DIR}/cubieboard2_defconfig
        (
            cd ${CB_KSRC_DIR}
